...
public class PlayerMovementScript : LivingEntity
{
    Rigidbody rb;
	...
	*/
    ...
    void OnHitObject(RaycastHit hit)
    {
        ...
        {
            damageableObject.TakeHit(meleeDamage);
        }
    }
	...
	*/
    ...
    void PrintHP()
    {
        int value = (int)(health / 100 * 255);
        if (value <= 0)
        {
            value = 0;
        }
        string color = Convert.ToString(value, 16);
        if (color == "0")
        {
            color = "00";
        }
        HP.GetComponent<Text>().text = $"<color=#ff{color}{color}>" + health + "</color>";
    }
