...
public class GunScript : MonoBehaviour
{
	...
	*/
    ...
    void CrossHairExpansionWhenWalking()
    {
        ...
        {//ifnot shooting
            ...
            expandValues_crosshair += new Vector2(5, 10) * Time.deltaTime;
            if (player.GetComponent<PlayerMovementScript>().maxSpeed < runningSpeed)
            { //not running
                expandValues_crosshair = new Vector2(Mathf.Clamp(expandValues_crosshair.x, 0, 2.5f), Mathf.Clamp(expandValues_crosshair.y, 0, 5));
	...
	*/
    ...
    {
        if (pmS.bulletsIHave > 0 && bulletsInTheGun < pmS.amountOfBulletsPerLoad && !reloading/* && !aiming*/)
        {
            ...
            if (meeleAttack == false && pmS.maxSpeed != runningSpeed)
            {
                ...
                if (pmS.bulletsIHave - pmS.amountOfBulletsPerLoad >= 0)
                {
                    pmS.bulletsIHave -= pmS.amountOfBulletsPerLoad - bulletsInTheGun;
                    bulletsInTheGun = pmS.amountOfBulletsPerLoad;
                }
                else if (pmS.bulletsIHave - pmS.amountOfBulletsPerLoad < 0)
                {
                    float valueForBoth = pmS.amountOfBulletsPerLoad - bulletsInTheGun;
                    if (pmS.bulletsIHave - valueForBoth < 0)
                    {
                        bulletsInTheGun += pmS.bulletsIHave;
                        pmS.bulletsIHave = 0;
                    }
                    else
                    {
                        pmS.bulletsIHave -= valueForBoth;
    ...
    void OnGUI()
    {
        bullets.GetComponent<Text>().text = pmS.bulletsIHave.ToString() + " - " + bulletsInTheGun.ToString();
        if (!HUD_bullets)
        {
            try
            {
        ...
        if (mls && HUD_bullets)
            HUD_bullets.text = pmS.bulletsIHave.ToString() + " - " + bulletsInTheGun.ToString();
